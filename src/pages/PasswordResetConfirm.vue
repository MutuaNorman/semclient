<script setup>
    ...
    const handleResetPasswordConfirm = async() => {
      try{
        ...
        const response = await axios.post("auth/users/reset_password_confirm/", resetData);
          if(response.status === 204){
            router.push({name:'login'})
            isLoading.value = false;
          }else if(response.status === 400){
              isLoading.value = false;
              message.value = "An error occurred, login again";
          }
      }catch(error){
        isLoading.value = false;
        if(error.response.data.non_field_errors){
          message.value = error.response.data.non_field_errors[0]
        }else if(error.response.data.password){
          message.value = error.response.data.password[0]
        }else{
          message.value = "An error occured, try again later.";
        }
      }
    }
          ...
          <form @submit.prevent="handleResetPasswordConfirm">
            <div class="reg-title">
              ...
              <div v-if="message" class="message">
                <i style="color:var(--custom-red)">{{ message }}</i>
              </div>
            </div>
            <div class="input">
                <input v-model="resetData.new_password" v-bind:type="showPassword ? 'text' : 'password'" id="password1" placeholder="New Password" />
            </div>
            <div class="input">
                <input v-model="resetData.re_new_password" v-bind:type="showPassword ? 'text' : 'password'" id="password2" placeholder="Confirm New Password" />
            </div>
